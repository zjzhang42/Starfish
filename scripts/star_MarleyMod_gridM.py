...
# adapted from "star_MarleyMod.py" but can now fit a metallicity grid
# ----
...
import argparse
parser = argparse.ArgumentParser(prog="star_MarleyMod_gridM.py", description="Run Starfish fitting model in single order mode with many walkers (applicable to a metallicity grid).")
...
# - Theta
theta_par = ThetaParam(grid=p0[0:3], vz=p0[3], vsini=p0[4], logOmega=p0[5])
theta_par.save()
# - Phi (including Cheb)
phi_par = PhiParam(spectrum_id, order_key, fix_c0, p0[6:9], p0[9], p0[10], p0[11])
phi_par.save()
# --
