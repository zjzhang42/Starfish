...
if args.config:
    ...
    import corner
    fig = corner.corner(flatchain_star[:,0:5], labels=param_keywords[:5], show_titles=True)
    fig.savefig(sfinfer_plotdir+'star_chain_corner.png', dpi=1000)

    # model flux
    mod_fls = lnprob_all(star_pars)

    # write an output data file containing observed and model spectra
    df_star = pd.DataFrame({'obs_wls':obs_wls, 'obs_fls':obs_fls, 'mod_fls':mod_fls})
