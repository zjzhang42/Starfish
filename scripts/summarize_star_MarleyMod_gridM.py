...
labels = [r'$T_{\mathrm{eff}}$', r'$\log{g}$', r'$Z$', r'$v_z$', r'$v\sin{i}$', r'$\log{\Omega}$',
          r'$c^1$', r'$c^2$', r'$c^3$', r'sigAmp', r'logAmp', r'$l$']
print_labels = ['Teff', 'log-g', 'Z', 'vz', 'vsini', 'logOmega', 'c1', 'c2', 'c3', 'sigAmp', 'logAmp', 'l']
default_sn_spl_id = 6  # stellar parameters for <5 and nuisance for >=5 in the final chain
# --
...
def store_star_MarleyMod(object, chain_file, resfile, specfile, f_burnin=0.5, num_noisedraw=1000, emucov=True, globcov=True, sigmacov=True, loccov=True):
    ...
    vz = final_param[:, 3]
    vsini = final_param[:, 4]
    logOmega = final_param[:, 5]
    Cheb_c1 = final_param[:, 6]
    Cheb_c2 = final_param[:, 7]
    Cheb_c3 = final_param[:, 8]
    nuis_sigAmp = final_param[:, 9]
    nuis_logAmp = final_param[:, 10]
    nuis_l = final_param[:, 11]
    ...
    resfile_load.create_dataset('Z', data=Z)
